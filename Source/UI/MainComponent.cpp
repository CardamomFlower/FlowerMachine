...
{
    // Esc = STOP ALL; Ctrl+S / Ctrl+O / Ctrl+N. Nothing else.
...
void MainComponent::confirmDiscard (std::function<void()> proceed, std::function<void()> cancelled)
{
    const auto giveUp = [cancelled] { if (cancelled != nullptr) cancelled(); };
    ...
    dialogs::askSaveChanges (controller.getPresetName(),
                             [safe = juce::Component::SafePointer<MainComponent> (this), proceed, giveUp] (dialogs::SaveChoice choice)
                             {
                                 if (safe == nullptr)
                                 {
                                     giveUp();
...
void MainComponent::requestClose (std::function<void()> proceed, std::function<void()> cancelled)
{
    confirmDiscard (std::move (proceed), std::move (cancelled));
}
