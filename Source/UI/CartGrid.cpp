...
{
    /*  A drag that ends while a dialog or a menu is up is never reported back here, so the
        drop mark would sit on a cell nothing is over any more. Nothing can be dropped on a
        grid that is modally blocked, so the mark has no business being there either.
    */
    if (hoverCell >= 0 && isCurrentlyBlockedByAnotherModalComponent())
        setHoverCell (-1);
