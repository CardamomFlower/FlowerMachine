...
/*  The FlowerMachine look, in two schemes.
    ...
    Both schemes keep every role the same: one amber accent for whatever is playing, red
    for STOP ALL and for trouble.
*/
namespace flowermachine::palette
{
    ...
    /** Segoe UI is on every Windows 10, so nothing has to be embedded or licensed. */
