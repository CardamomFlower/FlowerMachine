...
{
    /*  Main window content: top bar with the File menu,
    ...
    */
    ...
    public:
        ...
        */
        ...
        /** @param proceed   run when the operator saves or discards.
            @param cancelled run when they back out, so the caller can let go of any state it
                             took while the prompt was up. */
        void requestClose (std::function<void()> proceed, std::function<void()> cancelled = nullptr);
    ...
    private:
        ...
        void confirmDiscard (std::function<void()> proceed, std::function<void()> cancelled = nullptr);
