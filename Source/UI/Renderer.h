...
/*  Renderer preference.
    ...
    fallback for a machine whose graphics driver cannot. The choice is persisted in
    the settings; `--software-renderer` on the command line overrides it for one run.
*/
namespace flowermachine::renderer
{
    ...
    /*  Keeps every window on the chosen renderer.
        ...
        That is exactly the case the software switch exists for: the main window would
        paint and nothing else would. A slow poll is the only way to catch
