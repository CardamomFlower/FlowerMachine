...
namespace flowermachine
{
    ...
    public:
        ...
        {
            // Emergency switch for a window that does not paint at all.
            ...
            options.folderName = settingsFolderName;
            options.osxLibrarySubFolder = "Application Support";

            carryOverOldSettings (options);
        ...
        */
        void systemRequestedQuit() override
        {
            ...
            if (mainWindow != nullptr)
            {
                if (auto* main = dynamic_cast<MainComponent*> (mainWindow->getContentComponent()))
                {
                    /*  The prompt is asynchronous, so the flag cannot be cleared on the next
                        line: it has to be let go where the operator's answer arrives, or a
                        second request would stack a second prompt on top of the first.
                    */
                    quitRequested = true;
                    main->requestClose ([this] { quit(); }, [this] { quitRequested = false; });
                    return;
                }
            }
    ...
    private:
        static constexpr const char* settingsFolderName         = "CardamomFlower/FlowerMachine";
        static constexpr const char* previousSettingsFolderName = "CardamomTools/FlowerMachine";

        /*  Versions up to and including 0.3.0 kept the settings somewhere else. Nobody should lose their
            device choice, their colour scheme or their recent presets over a rename, so the old
            file is carried across once, on the first run that finds the new place empty. The old
            one is left where it is: this only ever reads it.
        */
        static void carryOverOldSettings (const juce::PropertiesFile::Options& options)
        {
            const auto current = options.getDefaultFile();

            if (current.existsAsFile())
                return;

            auto previous = options;
            previous.folderName = previousSettingsFolderName;
            const auto old = previous.getDefaultFile();

            if (old.existsAsFile())
            {
                current.getParentDirectory().createDirectory();
                old.copyFileTo (current);
            }
        }
        ...
        */
        ...
        public:
                  ...
                  settings (settingsToUse)
            {
                ...
                // saved on a large monitor is only nudged, never shrunk. On a smaller screen that
                // puts the title bar above the top edge with nothing left to grab.
                fitToDisplay();
