    ...
    : engine (engineToUse), loader (loaderToUse)
{
    ...
    startTimerHz (UI_REFRESH_HZ);   // sequence hand-over, and retire-list pruning
}
...
{
    // A pad pressed by hand takes over from any running sequence - but only a pad
...
{
    // Stopping the pad that is sounding ends the whole run - that is the panic
...
void Controller::valueTreeChildRemoved (juce::ValueTree& parent, juce::ValueTree& child, int index)
{
    ...
    {
        cancelSequence();
        engine.stopAll();   // the ids of every page after this one shift
...
void Controller::handleResult (const SampleLoader::Result& result)
{
    ...
    if (result.sample == nullptr)
    {
        ...
        engine.getCartEngine().unload (result.cartId);
