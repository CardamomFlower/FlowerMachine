...
void UninstallTask::run()
{
    ...
    /*  With no record in the registry the only other candidate is the folder this executable
        sits in - but it can have been copied anywhere, and step 2 below deletes the folder and
        everything under it. Take the guess only when the folder really is an installation, and
        refuse rather than delete something that merely happens to hold this file.
    */
    if (folder == juce::File() || ! folder.isDirectory())
    {
        const auto candidate = self.getParentDirectory();

        if (! candidate.getChildFile (EXE_NAME).existsAsFile())
        {
            fail ("Cannot tell where FlowerMachine was installed. Nothing was removed.");
            return;
        }
    ...
    {
        // Only the app's own settings folders - whatever sits above them may hold other
        // things and must survive.
        settingsFolder().deleteRecursively();
        previousSettingsFolder().deleteRecursively();
