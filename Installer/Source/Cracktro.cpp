...
namespace
{
    ...
    const char* const greetz =
        "FLOWERMACHINE INSTALLER   ***   A JINGLE MACHINE: A GRID OF PADS THAT PLAY THE MOMENT YOU CLICK   ***   "
...
void Cracktro::startAudio()
{
    ...
    // driver type synchronously, and one virtual audio driver publishing a long list of
    // WDM endpoints turns that into seconds of blank window. The screen paints first, this runs on the first tick.
    audioAttempted = true;
...
void Cracktro::beginShutdown()
{
    ...
    shuttingDown = true;
    synth.fadeOut();   // never cut an audio callback dead: that is a click out of the speakers
}
...
void Cracktro::buildImages()
{
    ...
    logoImage = juce::Image (format, WIDTH, 78, true);
    {
        ...
        g.drawText ("CARDAMOMFLOWER  -  " + juce::String (APP_VERSION), 0, 56, WIDTH, 18,
