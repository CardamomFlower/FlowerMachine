...
void writeFileAssociation (const juce::File& exe, const juce::File& icon)
{
    ...
    // An upgrade over a version that used the old ProgID would otherwise leave a second
    // handler registered for the same extension, both pointing at this same executable.
    removeLegacyProgId();
...
void removeLegacyProgId()
{
    removeKey (classesKey() + LEGACY_PROG_ID);

    if (Reg::getValue (extensionKey() + "\\") == LEGACY_PROG_ID)
        removeValue (extensionKey() + "\\");

    removeValue (extensionKey() + "\\OpenWithProgids\\" + LEGACY_PROG_ID);
}
